...
| PRD Plan READY 时 pack 满足最小可用性 | modules=7、FP=28、BR=4、api.has_api=true、endpoints=12 | 100% |
...
| 预期 | 模拟结果 | 实现率% |
|------|----------|---------|
| Split Plan Story 连续编号、slug 唯一 | Story 1..24；slug 唯一 | 100% |
| Split Plan Check API 阈值预检 | PASS | 100% |
| Split Generate 生成 24 个 story-N-slug.json | 文件名与 split-plan 1:1 | 100% |
| Split Check1 阈值门禁 | PASS | 100% |
| Split Check2 引用一致性 | PASS | 100% |
| Split Checkout 生成依赖图 | 生成 story-mermaid.md | 100% |

### 3.4 Story 执行（Story Check → Story Pack → Story Exec）

本次选中 Story 1（auth-foundation）进行执行验证。

| 预期 | 模拟结果 | 实现率% |
|------|----------|---------|
| Story Check 校验 YAML/引用一致性 | PASS | 100% |
| Story Pack 生成 exec-pack | 生成 story-exec/story-001-auth-foundation/ | 100% |
| Story Exec 只读 exec-pack | 禁止读 PRD/GC/story-*.json | 100% |
| Story Exec 关键文件覆盖 | server.py/storage.py/auth.py | 100% |
...
Story 1（基础登录与认证模块）的执行产出覆盖了 4 个功能点（FP-001/002/003/004）、3 个 API（API-001/002/003），验证命令 gate:compile 通过。
...
| docs/stories/story-001 ~ story-024 | 生成（24 个） |
| docs/split-check-index-pack.json | 生成 |
| docs/story-mermaid.md | 生成 |
| docs/story-exec/story-001-auth-foundation/ | 生成 |

### C. 生成的代码（Story 1）

| 文件 | 职责 |
|------|------|
| app/server.py | HTTP 服务器（登录/认证 API） |
| app/storage.py | SQLite 数据库层 |
| app/auth.py | 认证与会话管理 |
