...
1. 解析 `docs/story-$ARGUMENTS-exec-pack.yaml`：提取功能点、依赖、规则、数据变更、接口、验收标准、测试要求、验证命令
2. 若声明“前置Story/已有资源”：在仓库中 `rg` 定向检索，只读取必要签名（避免重复实现）
3. 实现：按验收标准最小改动完成需求，并补齐测试要求
4. 验证：按 `verification.commands` 顺序执行所有 `command != N/A` 的命令；任一失败则修复后重跑直到通过；若全部为 `N/A`：在输出中给出 `DECISION`（不猜命令；需用户补命令或手动验证）
