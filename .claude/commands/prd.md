...
通过友好对话引导用户澄清需求，最终按 `.claude/textum/PRD-framework-v1.md` 模板生成并冻结 `docs/PRD.md`。
...
9. 补齐接口设计（接口清单含“模块 M-xx”列 + 关键接口详情、错误码与失败场景）
10. 补齐非功能需求底线（安全/审计/性能/可观测；如不适用写 N/A）
11. 汇总复核 → 用户确认 → 标记 `docs/PRD.md` 为 Final 并冻结（之后不再修改）
...
- [ ] 接口清单已完整，且每条接口标注所属模块 `M-xx`（跨模块可多选）
- [ ] 关键接口详情包含：权限、请求/响应、失败场景（状态码+触发条件）
- [ ] 已给出非功能需求底线（安全/审计/性能/可观测；如不适用写 N/A）
- [ ] 用户已确认并同意冻结 `docs/PRD.md`（后续 Story 将引用精确行号）
