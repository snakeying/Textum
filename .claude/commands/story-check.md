...
2. 读取 PRD `### 9.2 接口清单`：按 `N/A_STRICT` 判断是否无 API（PRD_HAS_API）
...
- Story 必须包含 `关联功能点（必填）:` 行，且至少包含 1 个 `FP-001`
- FP ID 格式固定为 `FP-001`（3 位数字）；禁止 `FP-01`、`FP-xx`、`FP-##` 等
- 对 Story 中出现的每个 `FP-001`：必须能在 PRD `### 8.0 功能点→落点映射（必填）` 表中找到对应行；找不到即 `FAIL`
- `## 功能点（必填）` 章节内每条条目必须为 `- FP-001: ...`，且该章节出现的 `FP-001` 去重集合必须与“关联功能点”一致（不得缺失/多写）
...
- 在 PRD `### 8.0 功能点→落点映射（必填）` 表中抽取本 Story 关联 `FP-001` 对应行，解析“落点”列为集合（`N/A` 或逗号分隔多项）
- 期望集合（去重）：
...
- 若用户接受 `DECISION`：按 `PASS` 的下一步流程继续 `/story-pack $ARGUMENTS` → `/story $ARGUMENTS`；否则先修正后重跑 `/story-check $ARGUMENTS`
