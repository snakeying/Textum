...
1. **先读 Story 只抽取索引**：`模块`、`前置Story`、`GC#BR-###`、`API-###`、`TBL-###`、`PRD:Lx-Ly`
2. **再按索引定位 GC/PRD**：只查被引用的规则/ID/行号范围；不要通读全文
3. 输出只包含清单，不粘贴原文片段
...
### 4) 接口一致性（API-### 必须存在且匹配行号）

- 若 Story 的“接口”章节非 `N/A`：
  - 必须至少包含 1 个 `API-###`（具体数字：`API-001` 形式）
  - 每个 `API-###` 必须存在于 PRD `9.2 接口清单`
  - 若该条同时提供 `PRD:Lx-Ly`：校验该 PRD 行范围内包含该 `API-###` 字符串（避免“ID 与行号不匹配”）

### 5) 数据变更一致性（TBL-### 如适用）

- 若 Story 的“数据变更”章节非 `N/A`：
  - 若提供 `TBL-###` 且同时提供 `PRD:Lx-Ly`：校验该 PRD 行范围内包含该 `TBL-###` 字符串
