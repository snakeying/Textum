...
> 建议：当引用表/接口时，确保行范围覆盖其 `TBL-###` / `API-###` 标识所在行，便于 `/split-check` 做一致性校验。
...
为避免被 PRD 细节淹没，先“建索引、后回填行号”：

1. 先从 PRD 的索引章节建立集合：`5.1`（模块）、`6`（规则）、`8.1`（表ID/表名）、`9.2`（接口ID）
2. 先产出 Story 列表与依赖（只写模块/接口ID/表ID/规则ID，不写细节）
3. 再按 Story 逐个回到 PRD，用 `API-###` / `TBL-###` 精确定位到对应块，补齐 `PRD:Lx-Ly` 行号范围（避免全量通读）
...
4. 每个 Story 的 PRD 引用必须包含精确行号（数据/接口/规则）；接口必须附带 `API-###`（用于 `/split-check` 覆盖校验），数据表变更建议附带 `TBL-###`
