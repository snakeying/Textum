...
- PRD：
  ...
  - `8.0 功能点→落点映射`：用于把 `FP-001` 映射到本 Story 必须声明的 `PRD#TBL-###` / `ART:*`
...
3. 严格按模板填充：
   ...
   - `关联功能点（必填）`：填写本 Story 覆盖的 `FP-001`（必须是 PRD 中存在的具体数字；逗号分隔）
   - `## 功能点`：每条必须为 `- FP-001: ...`，且该章节出现的 `FP-001` 去重集合必须与“关联功能点”一致
