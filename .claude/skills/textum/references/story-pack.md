...
- 若 `FAIL`：
  - 只输出 `FAIL` 清单并停止（不写 pack 文件）
  - `FAIL` 清单：`F-001` 起编号；每条必须包含以下字段：
    - `定位`：来源文档 + 章节/锚点/表格行语义定位（尽量稳定）；避免行号
    - `问题`：1 句
    - `期望`：可机械执行的“替换目标/格式”（能推导就写出来）
    - `影响`：H/M/L
    - `修复`：只给 1 个动作（通常是“按定位修正来源文档后重跑 Story 执行包生成 $ARGUMENTS”）
  - 末尾追加：
    - `修正：按 FAIL 清单逐条修正来源文档后重跑 Story 执行包生成 $ARGUMENTS`
    - `重跑：Story 执行包生成 $ARGUMENTS`
- 若 `PASS`：
...
- `prd`（全部必须逐字原文粘贴）：
  ...
  2. 输出：
     ...
     - `下一步：Story 执行 $ARGUMENTS`

## 开始

请提供 Story 编号（例如：`1`）。我会写入 `docs/story-$ARGUMENTS-exec-pack.yaml`。
