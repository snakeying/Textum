...
- 对每个 `Story <n>` 的执行细则以 Story 执行为准（尤其是：硬约束 / 执行步骤 / 验证 / 低噪输出规则）
- 本命令仅改变：对多个 Story 依次执行并一次性汇总输出；不逐个等待人工验收

对 `N_list` 按顺序逐个执行：

1. 执行：
   - 对每个 `n`：按 Story 执行的硬约束与步骤执行（`docs/story-<n>-exec-pack.yaml` 为唯一事实来源；禁止再读取/通读 `docs/PRD.md` / `docs/GLOBAL-CONTEXT.md` / `docs/story-*.md`）
...
- 末尾追加：
  - `修正：按 FAIL 清单逐条修复后重跑 Story 批量执行（参数不变）`
  - `重跑：Story 批量执行（参数不变）`
