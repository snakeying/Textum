...
读取：`docs/PRD.md`（只读） | 写入：`docs/split-plan.yaml`（纯 YAML；不包含 ```） | 模板：`.claude/skills/textum/assets/split-plan-template.yaml` | 可选输入：`SPLIT_REPLAN_PACK`
...
- 输出必须严格按 `.claude/skills/textum/assets/split-plan-template.yaml` 的 YAML 结构（字段齐全；不得额外加 Markdown 文本；不得额外加键）
