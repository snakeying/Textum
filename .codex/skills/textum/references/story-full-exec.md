...
1. 执行：
   - 对每个 `n`：按“Story 执行”的硬约束与步骤执行（`docs/story-<n>-exec-pack.yaml` 为唯一事实来源；禁止再读取/通读 `docs/PRD.md` / `docs/GLOBAL-CONTEXT.md` / `docs/story-*.md`）
...
- 末尾追加：
  - `修正：按 FAIL 清单逐条修复后重跑 Story 批量执行 <原参数>`
  - `重跑：Story 批量执行 <原参数>`
