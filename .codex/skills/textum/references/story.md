...
5. 输出（低噪；不得粘贴大段代码/日志）：
   - `完成情况`：逐条列出 Story `## 验收标准` 的 `- [ ]` 项，并标记 `DONE` / `NOT_DONE`（若有未完成必须说明阻塞原因）
   - `关键变更`：仅列“应改动的关键文件路径 + 1 句职责/变化”，不贴 diff
   - `验证结果`：逐条列出执行过的 `gate:*` / `opt:*` 命令与结论（PASS/FAIL）；失败仅给 1 句摘要 + 下一步动作
   - `DECISION`：仅在需要用户确认时输出（例如：无可执行 gate 命令、opt 失败但可交付、或 pack 信息不足需回补）
   - `下一步`：给出下一条 Story 的入口（如无则写 `N/A`）
