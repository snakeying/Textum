...
def prd_render_labels(lang: str) -> dict[str, Any]:
    ...
    if lang == "zh":
        return {
            ...
            "sections": {
                ...
                "10": "## 10. 非功能需求（必填，可写 N/A）",
            },
            "blocks": {
                "feature_points": "**功能点（必填）**",
                "scenarios": "**关键场景/验收（必填）**",
                "table_constraints": "**关键约束（必填；无则写 N/A）**",
                "table_indexes": "**关键索引（必填；无则写 N/A）**",
                "request": "**请求（必填；无则写 N/A）**",
                "response": "**响应（必填；无则写 N/A）**",
                "failure": "**失败/边界（必填）**",
            },
            "tables": {
                "assumptions": ["ID", "假设/约束", "影响"],
                "roles": ["角色", "描述", "典型场景"],
                "permission_matrix": ["操作", "角色", "权限", "说明"],
            ...
            "table": {"purpose_one_liner": "**用途（必填，一句话）**"},
        }
    ...
    return {
        ...
        "sections": {
            ...
            "7": "## 7. States & Enums (Required)",
            "7.1": "### 7.1 Enums",
            "7.2": "### 7.2 State Machines (Required for flow entities; otherwise N/A)",
            "7.3": "### 7.3 Naming Conventions (Optional; otherwise N/A)",
            "8": "## 8. Data Model (Required)",
            ...
            "9": "## 9. API Design (Required)",
            "9.1": "### 9.1 Common Conventions (Required)",
            "9.2": "### 9.2 Endpoint List (Required)",
            "9.3": "### 9.3 Endpoint Details (one by one; locate via `API-###`)",
            "10": "## 10. Non-functional Requirements (Required; can be N/A)",
        },
        "blocks": {
            "feature_points": "**Feature Points (Required)**",
            "scenarios": "**Scenarios / Acceptance (Required)**",
            "table_constraints": "**Key Constraints (Required; otherwise N/A)**",
            "table_indexes": "**Key Indexes (Required; otherwise N/A)**",
            "request": "**Request (Required; otherwise N/A)**",
            "response": "**Response (Required; otherwise N/A)**",
            "failure": "**Failure / Edge Cases (Required)**",
        },
        "tables": {
            "assumptions": ["ID", "Assumption/Constraint", "Impact"],
            "roles": ["Role", "Description", "Typical Scenarios"],
            "permission_matrix": ["Operation", "Role", "Permission", "Note"],
        ...
        "table": {"purpose_one_liner": "**Purpose (Required; one sentence)**"},
    }
